...
const HOARetakenCourses = () => {
    ...
    const fetchRetakenCoursesData = async (mounted = true) => {
        ...
        try {
            ...
            // 2. Map Table Data (STRICTLY filtering for 'retake' records)
            if (assignmentsRes?.ok) {
                ...
                const parsedList = list
                    .filter(item => {
                        ...
                        const attempts = parseInt(item.total_attempts || 1);
                        // Define "Retake" logic: Status says retake OR attempts > 1
                        return status.includes('retake') || attempts > 1;
                    })
                    .map(item => ({
                        id: item.id || item._id,
                        studentName: item.student_name || 'Unknown Student',
                        location: item.location || '—',
                        flag: item.country_code === 'US' ? hoausflag : rwanda, // Fallback to rwanda
                        ...
                        attempts: item.total_attempts || '2', // Force default to 2 if it reached here without count
            ...
            console.error("Failed to fetch retaken courses", error);
            if (mounted) {
                setStatsData({});
    ...
    const processedData = useMemo(() => {
        let filtered = retakenCoursesData || [];
    ...
    }, [retakenCoursesData, sortConfig, searchQuery, selectedFilter]);
            ...
            <div className="hoa-retaken-courses-page hoa-assignments-page hoa-reports-page">
                ...
                <div className="hoa-page-header">
                    ...
                    <div className="hoa-header-actions">
                        <span className="hoa-update-status" onClick={() => fetchRetakenCoursesData(true)} style={{ cursor: 'pointer' }}>
                                    ...
                                    <td style={{ textAlign: 'center', position: 'relative' }}>
                                        ...
                                        {activeActionMenu === row.id && (
                                            <div className="rep-action-dropdown-menu">
                                                <div className="dropdown-item" onClick={() => handleAction(row.id, 'Approve Retake')}>Approve Retake</div>
